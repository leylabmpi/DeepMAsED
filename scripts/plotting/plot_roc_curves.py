...
def main(args):
    ...
    for model_path in path_to_models:
        F = os.path.join(args.save_path, model_path, 'final_model.h5')
        ...
        for itech, tech in enumerate(['megahit', 'metaspades']):
            ...
            #'---------------------------
            # ALE
            with open(os.path.join(args.ale_path, '../best_param_ale_train.pkl'),
                      'rb') as bestp:
        ...
        plt.legend([r'DeepMAsED (MH): AP=' + str(auc[0]), r'ALE (MH): AP=' + str(auc[1]), 
                    r'DeepMAsED (MS): AP=' + str(auc[2]), r'ALE (MS): AP=' + str(auc[3]),
                    'Random: AP=' + str(auc[4])], fontsize=17, 
                    loc='upper right')

        F = os.path.join(args.save_path, model_path, 'plots',
                         args.data_path.split('/')[-1], 'pr_curve.pdf')
        plt.savefig(F, bbox_inches='tight', format='pdf', dpi=5000)
