...
for mag in assemblies: 
    if not os.path.exists(os.path.join(args.assembly_path, mag, args.technology + '_all.pkl')):
        print("Pickle file not found for " + mag)
        exit()
    with open(os.path.join(args.assembly_path, mag, args.technology + '_all.pkl'), 'rb') as f:
        ale_scores.append(pickle.load(f))

thresh = {'depth': args.thr_len, 'place' : args.thr_place, 'insert' : args.thr_insert, 'kmer' : args.thr_kmer}
...
path_to_save = os.path.join(args.assembly_path, '../param_search_ap_' + args.assembly_path.split('/')[-1], args.technology + idx + '.pkl')
