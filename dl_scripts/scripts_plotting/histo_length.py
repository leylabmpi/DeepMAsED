...
ax.hist(all_lens[0], bins=np.logspace(np.log10(all_lens[0].min()), np.log10(all_lens[0].max()), 50), alpha=0.4, log=True, 
        label=r'MEGAHIT')
ax.hist(all_lens[1], bins=np.logspace(np.log10(all_lens[0].min()), np.log10(all_lens[0].max()), 50), alpha=0.4, log=True,
        label='MetaSPAdes')
plt.xscale('log')
